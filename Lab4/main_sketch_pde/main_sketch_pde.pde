...
void setup() {
    ...
    col = new Colony(100, 100, 1000, pherHome, pherFood);
...
void draw() {
    ...
    if (mousePressed && mouseButton == RIGHT) {  // Right-click to disperse
        for (int i = 0; i < col.ants.length; i++) {
            //col.ants[i].explosionForce = 100;
            col.ants[i].disperse(mouseX, mouseY);
        }
    }
    loadPixels();
